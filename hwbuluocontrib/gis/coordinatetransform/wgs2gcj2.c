...
   
bool wg2mars( const double in_lon, const double in_lat, double &out_lon, double &out_lat )
{
    const double BASE = 3686400.0;
    unsigned int lon, lat;
    lon = unsigned int ( in_lon * BASE );
    lat = unsigned int ( in_lat * BASE );
    unsigned int ret = wgtochina_lb( 1, lon, lat, 0, 0, 0, &lon, &lat );
    out_lon = lon / BASE;
    out_lat = lat / BASE;
    return (0==ret);
}
